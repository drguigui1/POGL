...
void Lights::add_directional_light(DirectionalLight light) {
    this->directional_light = light;
}

void Lights::add_point_light(PointLight light) {
    this->point_lights.push_back(light);
}

void Lights::send_data_to_shader(Shader& shader) {
    ...
    for (int i = 0; i < nb_p_lights; ++i) {
        ...
        shader.set_vec3("pointLights[" + str_i + "].pos", p_light.light_pos);
